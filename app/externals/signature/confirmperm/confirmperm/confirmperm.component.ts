...
@Component({
  selector: 'app-confirmperm',
  templateUrl: './confirmperm.component.html',
  styleUrls: ['./confirmperm.component.scss']
})
export class ConfirmpermComponent implements OnInit {

   id = this.route.snapshot.paramMap.get('id');  
   nom = this.route.snapshot.paramMap.get('nomPermanent');
   prenom = this.route.snapshot.paramMap.get('prenomPermanent');
   email = this.route.snapshot.paramMap.get('emailPermanent');
  ...
  ngOnInit() {
      ...
      this._SignatureService.getUrlConfirmPerm(this.id, this.nom, this.prenom, this.email, this.tel)
      .subscribe(data => {
         ...
         this.urlDownloadDoc = this.urlViewDoc+"/INSIDERS/rest/initiePermanent/downloadDoc/"+this.id;
         ...
         let mailModele={
          ...
          corps : " <style> \
            ...
            <table> \
              ...
              <tr>\
                <td>\
                <table width='100%'><tr><td style='width:30%;background:#fff;'></td><td style='width:30%;background:#000;'> <div> <a style='text-decoration:none; color:#FFFFFF;' href='"+this.urlViewDoc+"/INSIDERS/rest/initiePermanent/downloadDoc/"+this.id+"'> Consulter le document </a></div> </td><td  style='width:30%;background:#fff;'></td></tr></table>\
                </td> \
              </tr> \
            </table> \
            <table> \
