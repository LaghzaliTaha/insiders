...
@Component({
  selector: 'app-complete-initie-occasionnel',
  templateUrl: './complete-initie-occasionnel.component.html',
  styleUrls: ['./complete-initie-occasionnel.component.scss']
})
export class CompleteInitieOccasionnelComponent implements OnInit {
  fr:any;
  nomNaiss: string;
  addUserDisplayDialog= false;
  form: FormGroup;
  idInitieOccasionnel=  this.route.snapshot.paramMap.get('id');
  checkedForm = false;
  dateNaissance: Date;
  ...
  ngOnInit() {
    ...
    this._coreService.setcurrentPos('Modification Initié Occasionnel');
    ...
    this.form = this.formBuilder.group({
      ...
      referent:['', Validators.compose([Validators.required])],
      projets:['', Validators.compose([Validators.required])],
  ...
  createForm(): void {
    ...
    this._userService.getinitieOccasionnel(this.idInitieOccasionnel).subscribe(data => {
      this._userService.getprojetByInitieOcca(this.idInitieOccasionnel).subscribe(datas => {
    ...
    const dateNaiss=new Date(data.dateNaissance).getTime();
    // console.log('data.dateNaissance : ', data.dateNaissance);
    // console.log('data.dateSignature : ', data.dateSignature);
    // console.log('data.dateFinInscription : ', data.dateFinInscription);
    // console.log('data.dateInscription : ', data.dateInscription);
      ...
      this.form.patchValue({
       ...
       prenom: data.prenom,
       nom: data.nom,
       nomNaissance: data.nom,
      adresseMail: data.adresseMail, 
      dateNaissance:new Date(data.dateNaissance).getTime(),
      ...
      referent: data.pilote.prenom.concat('   '+ data.pilote.nom),
      projets: datas.nom,
      ...
      // dateSignature: dateSignature.toLocaleString(),
      // dateFinInscription: dateFinInscription.toLocaleString(),
      ...
      this.form.controls['referent'].disable();
      this.form.controls['projets'].disable();
...
this.form.get('dateNaissance').setValue(new Date(this.form.get('dateNaissance').value).getTime());
  const user= this.form.value;
   this._userService.updateinitieoccasionnel(this.idInitieOccasionnel,user).subscribe(
      resp => {
        //this.initiePerm=resp.data;
  
        const mailInitieOccasionnel = {
          objet:  'Formulaire remplit',
          corps : 'Bonjour, <br/> <br/> Merci d\'avoir renseign&eacute les informations exig&eacutees par l\'AMF. Vous disposez d\'un acc&egraves &agrave l\'application vous permettant de modifier vos informations &agrave tout moment '+InsidersConstants.APP_URI +InsidersConstants.ROUTE_INITIEOCCASIONNEL_REDIRECT+'/'+this.idInitieOccasionnel +' <br/> Cordialement',
          ...
          const mailPilote = {
            ...
            mailDestinataire: resp.pilote.adresseMail
          };
  
          
           this.addUserDisplayDialog=true;
          this._userService.sendSimpleMailUpdate(mailInitieOccasionnel).subscribe(
            res => { 
            },
            err => {
            });
          this._userService.sendSimpleMailUpdate(mailPilote).subscribe( res => { 
          },
          err => {
          });
